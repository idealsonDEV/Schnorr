...
func main() {
	...
	if !derivedPublicKey.Equal(publicKey) {
		panic("Derived public key is incorrect.")
	}
	if !Verify(message, signature, publicKey) {
		panic("Signature verification is incorrect. Signature does not yield correct public key.")
	}

	fakePublicKey := curve.Point().Mul(curve.Scalar().Neg(curve.Scalar().One()), publicKey)
	if Verify(message, signature, fakePublicKey) {
		panic("Signature verification is incorrect. Signature claims to belong to an incorrect public key.")
	}
}
